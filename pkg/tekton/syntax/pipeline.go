...
// Legacy code for Jenkinsfile generation
func (s *Step) ToJenkinsfileStatements() []*statement.Statement {
		...
		for k, v := range j.Options.PodLabels {
			...
			if sanitizedKey != k || sanitizedValue != v {
				log.Logger().Infof("Converted custom label/value '%s' to '%s' to conform to Kubernetes label requirements",
					termcolor.ColorInfo(k+"="+v), termcolor.ColorInfo(sanitizedKey+"="+sanitizedValue))
			}
...
func replacePlaceholdersInDir(originalDir *string, args StepPlaceholderReplacementArgs) *string {
	...
	dir = strings.Replace(dir, constants.PlaceHolderAppName, strings.ToLower(args.GitName), -1)
	dir = strings.Replace(dir, constants.PlaceHolderOrg, strings.ToLower(args.GitOrg), -1)
	dir = strings.Replace(dir, constants.PlaceHolderGitProvider, strings.ToLower(args.GitHost), -1)
	dir = strings.Replace(dir, constants.PlaceHolderDockerRegistryOrg, strings.ToLower(args.DockerRegistryOrg), -1)
